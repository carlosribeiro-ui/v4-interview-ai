    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1.5">
        {([['visaoGeral', 'Visão Geral'], ['porVaga', 'Por Vaga'], ['porTalent', 'Por Talent'], ['tabela', 'Tabela Completa']] as const).map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)} className={`px-3.5 py-1.5 rounded-full text-sm transition ${tab === id ? 'bg-v4red text-white font-medium' : 'bg-white/[0.05] text-white/60 hover:bg-white/10'}`}>{label}</button>
        ))}
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="rounded-xl border border-white/10 bg-white/5 p-4"><div className="text-xs text-white/50">Candidatos</div><div className="text-2xl font-bold">{data.totais.candidatos}</div></div>
        <div className="rounded-xl border border-white/10 bg-white/5 p-4"><div className="text-xs text-white/50">Concluídos</div><div className="text-2xl font-bold">{data.totais.concluidos}</div></div>
        <div className="rounded-xl border border-white/10 bg-white/5 p-4"><div className="text-xs text-white/50">Aprovados</div><div className="text-2xl font-bold text-v4green">{data.funil.aprovados}</div></div>
        <div className="rounded-xl border border-white/10 bg-white/5 p-4"><div className="text-xs text-white/50">Score médio</div><div className={`text-2xl font-bold ${corScore(data.totais.scoreMedioGeral)}`}>{data.totais.scoreMedioGeral?.toFixed(1) ?? '—'}</div></div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5">
            ...
            <div className="space-y-3">
              {data.vagas.map((v) => (
                <div key={v.id} className="flex items-center gap-3 text-sm p-2 rounded-lg hover:bg-white/[0.03]">
                  <div className="flex-1 min-w-0">
                    <div className="font-medium">{v.cargo}</div>
                    <div className="text-xs text-white/40">{v.senioridade}</div>
                  </div>
                  <div className="text-center px-3">
                    <div className="text-lg font-bold">{v.total}</div>
                    <div className="text-[10px] text-white/40">candidatos</div>
                  </div>
                  <div className="text-center px-3">
                    <div className="text-lg font-bold text-v4green">{v.concluidos}</div>
                    <div className="text-[10px] text-white/40">concluídos</div>
                  </div>
                  <div className="w-24">
                    ...
                    <tr key={t.email} className="border-b border-white/5 hover:bg-white/[0.03]">
                      ...
                      <td className="text-center py-2.5 px-2">
                        <div className="flex items-center gap-2 justify-center">
                          <div className="w-16 h-1.5 rounded-full bg-white/[0.06] overflow-hidden"><div className="h-full rounded-full bg-v4green" style={{ width: `${t.taxaConversao}%` }} /></div>
              ...
              <tbody>
                {data.topCandidatos.map((c, i) => (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/[0.03]">
                    ...
                    <td className="py-2 px-2 text-white/60 truncate">{c.vaga}</td>
                    <td className="py-2 px-2 text-center"><span className={`text-xs px-2 py-0.5 rounded-full ${c.fase === 'Aprovado' ? 'bg-v4green/15 text-v4green' : c.fase === 'Reprovado' ? 'bg-v4red/15 text-v4red' : 'bg-white/5 text-white/50'}`}>{c.fase}</span></td>
                    <td className="py-2 px-2 text-center"><span className={`font-semibold ${corScore(c.scoreMedio)}`}>{c.scoreMedio?.toFixed(1) ?? '—'}</span></td>
                    <td className="py-2 px-2 text-white/50 text-xs">{c.talent}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        return (
          <div key={l.label}>
            <div className="flex items-center justify-between text-sm mb-1">
              <span className="flex items-center gap-2 text-white/70"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: l.cor }} />{l.label}</span>
              <span className="text-white/90 font-semibold">{pct}% <span className="text-white/40 font-normal">· {l.valor}</span></span>
            </div>
            <div className="h-2 rounded-full bg-white/[0.06] overflow-hidden"><div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: l.cor }} /></div>
          </div>
        );
      })}
    </div>
...
function GraficoTimeline({ dados }: { dados: { periodo: string; total: number; concluidos: number }[] }) {
  if (dados.length === 0) return <p className="text-white/40 text-sm">Sem dados.</p>;
  const max = Math.max(1, ...dados.map((d) => d.total));
  return (
    <div className="h-36 flex items-end gap-1">
      {dados.map((d) => (
        <div key={d.periodo} className="flex-1 flex flex-col items-center gap-1 h-full justify-end group relative">
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 hidden group-hover:block bg-black/90 text-[10px] text-white px-2 py-1 rounded whitespace-nowrap z-10">{d.periodo}: {d.total} total, {d.concluidos} concluídos</div>
    ...
    <div className="space-y-3">
      {dados.map((d, i) => (
        <div key={i} className="flex items-center gap-3 text-sm">
          <div className="flex-1 min-w-0"><div className="font-medium truncate">{d.cargo}</div><div className="text-xs text-white/40">{d.senioridade} · {d.total} avaliados</div></div>
          <div className="w-24 h-2 rounded-full bg-white/[0.06] overflow-hidden shrink-0"><div className="h-full rounded-full" style={{ width: `${((d.scoreMedio ?? 0) / 10) * 100}%`, backgroundColor: (d.scoreMedio ?? 0) >= 7 ? '#52CC5A' : (d.scoreMedio ?? 0) >= 4 ? '#FFC02A' : '#E50914' }} /></div>
